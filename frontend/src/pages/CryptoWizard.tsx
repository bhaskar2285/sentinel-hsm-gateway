    ...
    <div className="max-w-5xl space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            End-to-end Thales lifecycle: generate ZMK → wrap ZPK under it → export TR-31 → encrypt → decrypt round-trip.
        ...
        <div className="flex gap-2">
          ...
          <button onClick={runAll} className="btn-primary" disabled={!!busy}>
      ...
      <ol className="space-y-3">
        {STEPS.map((step, idx) => {
          ...
          return (
            <li key={step.key} className={`card overflow-hidden transition-colors ${done ? 'border-emerald-200' : ''}`}>
              <button
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="chip-mono">{step.cmd}</span>
                    <span className="text-[10px] text-slate-400 font-mono">{step.spec}</span>
                  </div>
              ...
              {open && (
                <div className="px-5 pb-5 border-t border-slate-200 bg-slate-50/50">
                  <div className="pt-4 flex gap-2">
                    <button onClick={() => run(step.key)} disabled={running}
                            className="btn-primary text-xs">
                      {running ? <><Loader2 size={12} className="animate-spin" /> Running…</> : <>Run {step.cmd}</>}
                    </button>
                  </div>
      ...
      {state.decrypt && (
        <div className={`card px-5 py-4 ${finalRoundtripOk ? 'border-emerald-300 bg-emerald-50' : 'border-rose-300 bg-rose-50'}`}>
          <div className="flex items-center gap-2">
            <Check size={16} className={finalRoundtripOk ? 'text-emerald-600' : 'text-rose-600'} />
  ...
  if (stepKey === 'exportTr31' && state.tr31) {
    return (
      <div className="mt-4 space-y-1">
        <div className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">TR-31 Format D Key Block</div>
        <pre className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-mono break-all whitespace-pre-wrap">
{state.tr31.keyBlock}
        </pre>
        <div className="text-[10px] text-slate-500 mt-1">
          ASCII envelope: <code className="chip-mono">D0096</code> = version D, 96 bytes ·
          <code className="chip-mono ml-1">P0TE</code> = PIN-encrypt usage, 3DES, encrypt-only, exportable ·
          MAC bound via AES-CMAC.
        </div>
      </div>
    );
  }
    ...
    <dl className="mt-4 grid grid-cols-[140px_1fr] gap-x-4 gap-y-1.5 text-xs">
      {rows.map(([k, v]) => (
        <>
          ...
          <dd className="font-mono break-all text-slate-900">{v}</dd>
        </>
      ))}
    </dl>
  );
